...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Crear los datos para el pedido, que incluyen los detalles (pedidoItems)
    const pedidoData = {
      ...formData,
    ...
    if (selectedItemId) {
      if (selectedItemId) {
        response = await axios.put(
          `http://localhost:8080/api/${activeSection}/${selectedItemId}`,
          pedidoData
        );
      } else {
        response = await axios.post(
          `http://localhost:8080/api/${activeSection}`,
          pedidoData
        );
      }
  
      // Actualizar el estado local basado en el tipo de sección
      ...
      response = await axios.post(
        `http://localhost:8080/api/${activeSection}`,
        pedidoData // Enviar pedidoData, que incluye los detalles del pedido
      );
  ...
  const [pedidoItems, setPedidoItems] = useState([]); // Estado para almacenar los items seleccionados con cantidad

  const handleItemQuantityChange = (itemId, quantity) => {
    // Verificar si el item ya existe en el estado de pedidoItems
    setPedidoItems((prevItems) => {
      const itemIndex = prevItems.findIndex((item) => item.id === itemId);
      if (itemIndex !== -1) {
        // Si el item ya está en la lista, actualizar la cantidad
        const updatedItems = [...prevItems];
        updatedItems[itemIndex].cantidad = quantity;
        return updatedItems;
      } else {
        // Si el item no está en la lista, agregarlo
        return [...prevItems, { id: itemId, cantidad: quantity }];
      }
    });
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        {activeSection === 'itemsMenu' && (
          <>
            ...
            {/* Mostrar campos específicos para comida */}
            {formData.esComida && (
              <>
                <input
                  type="number"
                ...
                <div>
                  <label>
                    <input
                      type="checkbox"
                      name="aptoCeliaco"
                      checked={formData.aptoCeliaco || false}
                      onChange={(e) => setFormData({ ...formData, aptoCeliaco: e.target.checked })}
                    />
                    Apto celiaco
                  </label>
                </div>
                <div>
                  <label>
                    <input
                      type="checkbox"
                      name="aptoVegano"
                      checked={formData.aptoVegano || false}
                      onChange={(e) => setFormData({ ...formData, aptoVegano: e.target.checked })}
                    />
                    Apto vegano
                  </label>
                </div>
            ...
            {/* Mostrar campos específicos para bebida */}
            {!formData.esComida && (
              <>
                <input
                  type="number"
        ...
        {activeSection === 'pedidos' && (
            <>
              ...
              {/* Selección de Items del Menú con cantidades */}
              <h3>Seleccionar Items del Menú</h3>
              {itemsMenu.map((item) => (
                <div key={item.id}>
                  <label>
